function [MAP_COMPUTED] = combine_computed_v3(MOLUZ,M11,MSHE,OL,M3DSZQ,M3DSZ,INI)
...
DATE_I = INI.ANALYZE_DATE_I;
DATE_F = INI.ANALYZE_DATE_F;
...
TIMEVECTOR = [DATE_I_NUM:DATE_F_NUM]';
KEYS_STRINGS = {};
...
if length(KEYS_STRINGS) == 0
  fprintf('... ERROR in combine_computed - NO KEYS FOUND FOR COMPUTED DATA\n');
  return;
end
...
for KEY = NAME_KEYS
    ...
    for i = 1:sz
        ...
        DATA_COMPUTED(ii).RUN{i} = INI.MODEL_SIMULATION_SET{i}{3};
