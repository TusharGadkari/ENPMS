function [ MAP_ALL_DATA ] = combine_observed_timeseries_v1_BETA(MAP_ALL_DATA,obs_index)
...
try
  ...
  if isKey(MAP_ALL_DATA,mykey)
     ...
     NEW_STRUCTURE.RUN        = MAP_ALL_DATA(mykey).RUN;
  ...
  % otherwise copy datatype,unit and time vector from one component
     ...
     NEW_STRUCTURE.RUN        = MAP_ALL_DATA(copykey).RUN;
  end
...
try
	% attempt to access and sum up component timeseries
	NEW_STRUCTURE.TIMESERIES(:,obs_index) = ...
	       MAP_ALL_DATA(char('L31NMILE1_Q')).TIMESERIES(:,obs_index) + ...
	(-1.0)*MAP_ALL_DATA(char('L31NMILE7_Q')).TIMESERIES(:,obs_index) ;
	
  % if exists copy model data to new variable and delete old one
  if isKey(MAP_ALL_DATA,mykey)
     NEW_STRUCTURE.TIMEVECTOR = MAP_ALL_DATA(mykey).TIMEVECTOR;
     NEW_STRUCTURE.DATATYPE   = MAP_ALL_DATA(mykey).DATATYPE;
     NEW_STRUCTURE.DFSTYPE    = MAP_ALL_DATA(mykey).DFSTYPE;
...
try
	...
	NEW_STRUCTURE.TIMESERIES(:,obs_index) = ...
	       MAP_ALL_DATA(char('L31NMILE1_Q')).TIMESERIES(:,obs_index) + ...
	(-1.0)*MAP_ALL_DATA(char('L31NMILE3_Q')).TIMESERIES(:,obs_index) ;
...
try
	% attempt to access and sum up component timeseries
	NEW_STRUCTURE.TIMESERIES(:,obs_index) = ...
	       MAP_ALL_DATA(char('L31NMILE3_Q')).TIMESERIES(:,obs_index) + ...
	(-1.0)*MAP_ALL_DATA(char('L31NMILE4_Q')).TIMESERIES(:,obs_index) ;
...
try
	% attempt to access and sum up component timeseries
	NEW_STRUCTURE.TIMESERIES(:,obs_index) = ...
	       MAP_ALL_DATA(char('L31NMILE4_Q')).TIMESERIES(:,obs_index) + ...
	(-1.0)*MAP_ALL_DATA(char('L31NMILE5_Q')).TIMESERIES(:,obs_index) ;
...
try
	% attempt to access and sum up component timeseries
	NEW_STRUCTURE.TIMESERIES(:,obs_index) = ...
	       MAP_ALL_DATA(char('L31NMILE5_Q')).TIMESERIES(:,obs_index) + ...
	(-1.0)*MAP_ALL_DATA(char('L31NMILE7_Q')).TIMESERIES(:,obs_index) ;
