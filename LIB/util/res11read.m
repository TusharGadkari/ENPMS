function [vals, outInfos] = res11read(infile, extractPoints)
...
for ii = 1:numel(extractPoints)
   ...
   % Looping over possible several chainage values
   for ch = extractPoints{ii}.chainages
      ...
      for iitem = 1:numel(itemInfos)
         ...
         if (~strcmpi(extractPoint.branchName,itemInfo.branchName)) 
             continue; % branc name does not match
         end
         [minch,minchI] = min(abs(itemInfo.chainages-ch));
         if (minch > 1)
      ...
      if (~found)
          fprintf('Did not find extract point %i, chainage %d\n',ii,ch);
      end
   end
end
