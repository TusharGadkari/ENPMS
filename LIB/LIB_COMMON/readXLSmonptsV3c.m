function [MAP_ALL] = readXLSmonptsV3c(MAKETEXTFILE,xlsfile)
...
if (regexp(char(address), 'ENP-PC'))
    ...
    %path(path,workdir);
elseif (regexp(char(address), 'ZENP'))
    pathdir     = 'F:/MODELS/DHIMODEL/';
    matdir =  'F:/MODELS/MATLAB/';
    path(path, 'C:\Program Files\MATLAB\R2011a\mbin');
    path(path,[matdir 'scripts/common']);
    %outdir = [matdir 'postproc/data/'];
else
...
if (~exist('MAKETEXTFILE','var')), MAKETEXTFILE = 0; end
    %output file
if (~exist('printname','var')),printMSHEname = [matdir 'DATASTRUCTURES/detTSmsheALL.txt']; end
xlsMSHE = 'monpts';
xlsMSHEadd = 'monptsadd';
dfs0MSHEdir=[pathdir 'INPUTFILES/MSHE/TIMESERIES/'];
dfs0MSHEdpthdir= [pathdir 'INPUTFILES/MSHE/TSDEPTH/'];
if (~exist('printname','var')),printM11name = [matdir 'DATASTRUCTURES/detTSm11ALL.txt']; end
xlsM11 = 'M11';
xlsM11add = 'M11add';
