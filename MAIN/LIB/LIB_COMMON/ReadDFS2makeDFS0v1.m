function [TS] = ReadDFS2makeDFS0v1(TS,dfs2,itm)
...
for i=0:TS.S.nsteps-1
	...
	h = double(dfs2.ReadItemTimeStep(itm,i).To2DArray());
    hall(i+1,:,:) = h;
    %htot = htot+ h;
end
...
TS.ValueVector =  TSmerge(hall(:,TS.col,TS.row), TS.dlength, datenum(TS.startdate), datenum(TS.enddate), dfsstartdatetime, DfsTime);
