function [MAP_ALL] = readXLSmonptsV2(MAKETEXTFILE,xlsfile)
...
if (~exist('MAKETEXTFILE','var')), MAKETEXTFILE = 0; end
    %output file
if (~exist('printname','var')),printMSHEname = [pathdir 'DATAOBS/RAWDATA/detTSmsheALL.txt']; end
xlsMSHE = 'monpts';
xlsMSHEadd = 'monptsadd';
dfs0MSHEdir=[pathdir 'INPUTFILES/MSHE/TIMESERIES/'];
dfs0MSHEdpthdir= [pathdir 'INPUTFILES/MSHE/TSDEPTH/'];
if (~exist('printname','var')),printM11name = [pathdir 'DATAOBS/RAWDATA/detTSm11ALL.txt']; end
xlsM11 = 'M11';
xlsM11add = 'M11add';
dfs0M11dir=[pathdir 'INPUTFILES/M11/TIMESERIES/'];
...
row = xldata(1:numrows,6);
col = xldata(1:numrows,7);
dfegse =  xldata(1:numrows,8);
gridgse = xldata(1:numrows,9);
...
row = xldata(1:numrowsadd,6);
col = xldata(1:numrowsadd,7);
dfegse =  xldata(1:numrowsadd,8);
gridgse = xldata(1:numrowsadd,9);
for i = 1:numrowsadd
