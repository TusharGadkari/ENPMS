...
INI.SELECTED_STATIONS = get_station_listV2(INI.SELECTED_STATION_LIST);
STATIONS_LIST = INI.SELECTED_STATIONS.list.stat;
%STATIONS_LIST = {'NP205'};

% The no. of runs from matlab file plus observed 
numTS = length(DATA.MAP_ALL_DATA(char(STATIONS_LIST(1))).RUN) + 1;
...
mapstarttime=DATA.MAP_ALL_DATA(char(STATIONS_LIST(1))).TIMEVECTOR(1);
mapendtime=DATA.MAP_ALL_DATA(char(STATIONS_LIST(1))).TIMEVECTOR(end);
...
for M = STATIONS_LIST
    ...
    plotboxcomb(OUT, numTS, minvl, maxvl, legnd, plotfile,STT,gse,DATASTATION);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [output] = plotboxcomb(OUT, numTS,minvl, maxvl, legnd, plotfile,STT,gse,STATION)
